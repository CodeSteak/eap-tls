...
mod test {
    ...
    fn auth_md5_challenge_method() {
        ...
        assert!(matches!(res, ThisLayerResult::Send(MessageContent { data }) if data.len() == 16+1));

        { // Wrong Response
            let mut method = method.clone();
            let m = Message::new(crate::message::MessageCode::Response, 0, b"bob");
            assert_eq!(
                method.recv(&[0x10, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16], &RecvMeta { // <- LOL Hex counting
                    message: &m,
        ...
        method.set_challange_data(&crate::util::hex_to_vec("c7 24 0a c7 c5 94 58 ee 8f 7a 98 35 6c 61 53 f0"));

        let m = Message::new(crate::message::MessageCode::Response,  0x69 /* <- part of hash */, b"");
        assert_eq!(
            method.recv(
